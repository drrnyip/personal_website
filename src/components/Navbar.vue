      ...
      <div class="relative flex items-center justify-between h-16">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          <button type="button" class="bg-gray-800 p-1 rounded-full text-gray-400 hover:text-white">
            ...
            <LightBulbIcon v-if="darkMode" class="h-6 w-6" @click="emitDarkMode" aria-hidden="true" />
            <LightBulbSolid v-if="!darkMode" class="h-6 w-6" @click="emitDarkMode" aria-hidden="true" />
          </button>
        </div>
      </div>
    </div>
